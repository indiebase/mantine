...
```tsx
import { ColorSwatch } from '@reactgets/core';
...
as a third argument. Use `getStylesRef` exported from `@reactgets/core` package instead:
...
### @reactgets/notifications breaking changes

[@reactgets/notifications](https://mantine.dev/others/notifications/) package no longer exports
...
### @reactgets/rte package deprecation

`@reactgets/rte` package is deprecated – it will no longer receive updates (last version will remain 5.x)
and it may no longer be compatible with `@reactgets/core` and `@reactgets/hooks` packages (6.x and later versions).
Migrate to [@reactgets/tiptap](https://mantine.dev/others/tiptap/) as soon as possible.

### @reactgets/dates breaking changes

All components from `@reactgets/dates` package were rebuilt from scratch.
...
### @reactgets/spotlight breaking changes
...
- `@reactgets/next` package no longer exports `NextLink` component
...
- `@reactgets/core` package no longer exports `GroupedTransition` component, use multiple [Transition](https://mantine.dev/core/transition/) components instead
...
## Updated @reactgets/dates package

`@reactgets/dates` package was rebuilt from scratch, it now includes new components to
...
components exported from `@reactgets/dates` package:
...
## @reactgets/spotlight improvements
...
- [@reactgets/notifications](https://mantine.dev/others/notifications/) package now exports `notifications` object that includes functions to show, update, hide, clean and clean queue
- [@reactgets/nprogress](https://mantine.dev/others/nprogress/), [@reactgets/modals](https://mantine.dev/others/modals/) and [@reactgets/spotlight](https://mantine.dev/others/spotlight/) packages now exports `nprogress`, `modals` and `spotlight` objects with all package methods
...
- [@reactgets/form](https://mantine.dev/form/validators/) now exports new `matchesField` validator
