...
<PackagesInstallation extraPackages={['@reactgets/next', '@emotion/server']} />

Create `pages/_document.tsx` file:

```tsx
import { createGetInitialProps } from '@reactgets/next';
...
import { ServerStyles, createStylesServer } from '@reactgets/next';
