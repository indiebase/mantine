...
Each `@reactgets/` package that exports components, exports props types for these components as well.
...
```tsx
import type { ButtonProps } from '@reactgets/core';
import type { DatePickerProps } from '@reactgets/dates';
```
...
```tsx
import { createStyles, Selectors } from '@reactgets/core';
...
```tsx
import { MantineTheme, useMantineTheme } from '@reactgets/core';
...
Other types exported from `@reactgets/core` package:
