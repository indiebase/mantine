...
description: 'Settings provider for @reactgets/dates components'
import: "import { DatesProvider } from '@reactgets/dates';"
source: 'mantine-dates/src/components/DatesProvider/DatesProvider.tsx'
docs: 'dates/DatesProvider.mdx'
---

import { DatesProviderDemos } from '@reactgets/demos';

## Usage

`DatesProvider` component lets you set various settings that are shared across all
components exported from `@reactgets/dates` package. `DatesProvider` supports the following settings:
