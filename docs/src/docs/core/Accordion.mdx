...
import: "import { Accordion } from '@reactgets/core';"
source: 'mantine-core/src/Accordion/Accordion.tsx'
docs: 'core/Accordion.mdx'
styles: ['Accordion']
---

import { AccordionDemos } from '@reactgets/demos';
...
```tsx
import { Accordion, rem } from '@reactgets/core';
...
`AccordionProps` type exported from `@reactgets/core` is a generic, it accepts boolean type that
describes `multiple` state:

```tsx
import type { AccordionProps } from '@reactgets/core';
