...
`@reactgets/core` package exports `rem` and `em` function that can be used to convert `px` into `rem`/`em`:

```tsx
import { rem, em } from '@reactgets/core';
...
To convert `rem` to `px` use `px` function exported from `@reactgets/core`:

```tsx
import { px } from '@reactgets/core';
