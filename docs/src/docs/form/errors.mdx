...
`form.errors` type is `Record<string, React.ReactNode>`, you can import a shorthand `FormErrors` type from `@reactgets/form`:

```tsx
import type { FormErrors } from '@reactgets/form';
```
