...
import: "import { SpotlightProvider } from '@reactgets/spotlight';"
source: 'mantine-spotlight/src'
docs: 'others/spotlight.mdx'
installation: '@reactgets/spotlight'
license: MIT
styles: ['SpotlightProvider']
---

import { SpotlightDemos } from '@reactgets/demos';
...
```bash
yarn add @reactgets/spotlight
```

Install with npm:

```bash
npm install @reactgets/spotlight
```
...
`@reactgets/spotlight` exports `spotlight` object with functions which can be used to perform certain actions
from any part of your application:

```tsx
// All functions can be called from anywhere (not only from components)
import { spotlight } from '@reactgets/spotlight';
...
You can import `SpotlightAction` type from `@reactgets/spotlight` package:
...
```tsx
import { RemoveScroll } from '@reactgets/core';
