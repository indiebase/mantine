...
import: "import { RichTextEditor } from '@reactgets/tiptap';"
source: 'mantine-tiptap/src'
docs: 'others/tiptap.mdx'
installation: '@reactgets/tiptap'
license: MIT
styles: ['RichTextEditor']
---

import { TipTapDemos } from '@reactgets/demos';
...
```bash
yarn add @reactgets/tiptap @reactgets/core @reactgets/hooks @tabler/icons-react @tiptap/react @tiptap/pm @tiptap/extension-link @tiptap/starter-kit
```

Install with npm:

```bash
npm install @reactgets/tiptap @reactgets/core @reactgets/hooks @tabler/icons-react @tiptap/react @tiptap/pm @tiptap/extension-link @tiptap/starter-kit
```

## TipTap editor

`@reactgets/tiptap` provides a UI for [Tiptap](https://tiptap.dev/). `RichTextEditor` component
...
`@reactgets/tiptap` provides custom `Link` extension that is required to be used instead of
`@tiptap/extension-link` in order for `Ctrl + K` keyboard shortcut to work:

```tsx
// Use Link extension exported from the @reactgets/tiptap package
...
```tsx
import { useRichTextEditorContext } from '@reactgets/tiptap';
...
```tsx
// RichTextEditorLabels type can be imported from @reactgets/tiptap package
...
Default labels (can be imported from `@reactgets/tiptap` package):
