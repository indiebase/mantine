...
import: "import { Carousel } from '@reactgets/carousel';"
source: 'mantine-carousel/src'
docs: 'others/carousel.mdx'
installation: '@reactgets/carousel'
styles: ['Carousel']
license: MIT
---

import { CarouselDemos } from '@reactgets/demos';
...
```bash
yarn add embla-carousel-react @reactgets/carousel
```

Install with npm:

```bash
npm install embla-carousel-react @reactgets/carousel
```

## Usage

`@reactgets/carousel` package is based on [embla carousel](https://www.embla-carousel.com/),
...
Note that plugins are not installed with `@reactgets/carousel` package and you will need to
...
To solve this issue use `useAnimationOffsetEffect` hook exported from `@reactgets/carousel` package.
