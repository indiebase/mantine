...
import: "import { Dropzone } from '@reactgets/dropzone';"
source: 'mantine-dropzone/src'
docs: 'others/dropzone.mdx'
installation: '@reactgets/dropzone'
license: MIT
...
} from '@reactgets/dropzone';
import { DropzoneDemos } from '@reactgets/demos';
...
```bash
yarn add @reactgets/dropzone
```

Install with npm:

```bash
npm install @reactgets/dropzone
```
...
To save some research time you can use `MIME_TYPES` variable exported from `@reactgets/dropzone`:

```tsx
import { Dropzone, MIME_TYPES } from '@reactgets/dropzone';
...
```tsx
import { IMAGE_MIME_TYPE, Dropzone } from '@reactgets/dropzone';
